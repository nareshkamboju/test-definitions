...
install() {
	...
	case "${dist}" in
		debian|ubuntu)
			pkgs="git libtool automake build-essential curl python3 m4 autoconf-archive libkmod-dev libudev-dev catch2"
			install_deps "${pkgs}" "${SKIP_INSTALL}"
			;;
		fedora|centos)
			pkgs="git-core libtool automake make gcc gcc-c++ curl python3 m4 autoconf-archive libkmod-dev libudev-dev catch2"
			install_deps "${pkgs}" "${SKIP_INSTALL}"
			;;
